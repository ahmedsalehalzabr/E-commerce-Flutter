...
import 'package:numo/Screen/MyOrder.dart';
...
class PushNotificationService {
  ...
  Future initialise() async {
    ...
    messaging.getToken().then((token) async {
      ...
      if (settingsProvider.merchant_id != null && settingsProvider.merchant_id != "") {
      ...
      if (type == "ticket_status") {
        ...
        if (CUR_TICK_ID == id) {
          if (chatstreamdata != null) {
            ...
            chat["data"] = sendata;
            if (parsedJson[USER_ID] != settingsProvider.merchant_id) {
    ...
    messaging.getInitialMessage().then((RemoteMessage? message) async {
      ...
      bool back = await Provider.of<SettingProvider>(context, listen: false).getPrefrenceBool(ISFROMBACK);
  ...
  void _registerToken(String? token) async {
    SettingProvider settingsProvider = Provider.of<SettingProvider>(context, listen: false);
    var parameter = {MERCHANT_ID: settingsProvider.merchant_id, FCM_ID: token};

    Response response = await post(updateFcmApi, body: parameter, headers: headers).timeout(const Duration(seconds: timeOut));
  ...
  Future<void> getProduct(String id, int index, int secPos, bool list) async {
    try {
      ...
      Response response = await post(getProductApi, headers: headers, body: parameter).timeout(const Duration(seconds: timeOut));
      var getdata = json.decode(response.body);
...
Future<dynamic> myForgroundMessageHandler(RemoteMessage message) async {
  ...
  log('Main myForgroundMessageHandler ');
