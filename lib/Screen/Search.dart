...
class _SearchState extends State<Search> with TickerProviderStateMixin {
  ...
  Widget listItem(int index) {
    ...
    _controllerList[index].text = model.ProductAttributeValues![model.selVarient!].cartCount!;
                                                  ...
                                                  InkWell(
                                                    ...
                                                    onTap: () {
                                                      if (_isProgress == false &&
                                                          (int.parse(productList[index].ProductAttributeValues![model.selVarient!].cartCount!)) > 0) {
                                                    ...
                                                    child: Stack(
                                                      children: [
                                                        TextField(
                                                          ...
                                                          readOnly: true,
                                                          style: TextStyle(fontSize: 10, color: Theme.of(context).colorScheme.fontColor),
                                                          controller: _controllerList[index],
                                                          decoration: InputDecoration(
                                                            contentPadding: const EdgeInsets.all(5.0),
                                                            focusedBorder: OutlineInputBorder(
                                                              borderSide: BorderSide(color: Theme.of(context).colorScheme.fontColor, width: 0.5),
    ...
    if (_isNetworkAvail) {
      if (CUR_MERCHANTID != null) {
        try {
          ...
          qty = (int.parse(productList[index].ProductAttributeValues![model.selVarient!].cartCount!) - int.parse(productList[index].qtyStepSize!));
  ...
  void getAvailVarient(List<Product> tempList) {
    ...
    if (notificationoffset == 0 && buildResult) {
      ...
      for (int i = 0; i < history.length; i++) {
        if (history[i].product_name == query) productList.insert(0, history[i]);
      }
    }
  ...
  _tags() {
    if (tagList != null) {
      ...
      for (int i = 0; i < tagList.length; i++) {
        tagChip = ChoiceChip(
          selected: false,
          label: Text(tagList[i], style: TextStyle(color: Theme.of(context).colorScheme.white, fontSize: 11)),
        ...
        chips.add(Padding(padding: const EdgeInsets.symmetric(horizontal: 2), child: tagChip));
      }
...
class _SuggestionList extends StatelessWidget {
  const _SuggestionList({this.suggestions, this.textController, this.searchDelegate, this.notificationcontroller, this.getProduct, this.clearAll});
    ...
    return ListView.separated(
      ...
      itemBuilder: (BuildContext context, int i) {
        ...
        return ListTile(
            ...
            subtitle: textController!.text.toString().trim().isEmpty || suggestion.history!
                ? null
                : Text(
            ...
            leading: textController!.text.toString().trim().isEmpty || suggestion.history!
                ...
                : ClipRRect(
                    borderRadius: BorderRadius.circular(7.0),
                    child: suggestion.product_image == ''
                        ? Image.asset(
                        ...
                        : FadeInImage(
                            image: NetworkImage(suggestion.product_image!),
            ...
            onTap: () async {
              if (suggestion.product_name!.startsWith('Search Result for ')) {
                SettingProvider settingsProvider = Provider.of<SettingProvider>(context, listen: false);

                settingsProvider.setPrefrenceList(HISTORYLIST, textController!.text.toString().trim());
                ...
                buildResult = false;
                textController!.text = suggestion.product_name!;
