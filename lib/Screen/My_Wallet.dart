...
import 'package:numo/Screen/PaypalWebviewActivity.dart';
...
class StateWallet extends State<MyWallet> with TickerProviderStateMixin {
                        ...
                        child: Column(mainAxisAlignment: MainAxisAlignment.start, children: [
                              ...
                              child: TextButton(
                                  child: Text(getTranslated(context, 'WAL_TRANS_LBL')!,
                                      style: Theme.of(context)
                                          .textTheme
                                          .subtitle1!
                                          .copyWith(color: Theme.of(context).colorScheme.fontColor.withOpacity(0.5))),
                                  onPressed: () {
                                    setState(() {
                                      isWithdraw = false;
                                      isLoadingmore = true;
                          ...
                          Padding(
                            ...
                            child: Container(
                              ...
                              child: TextButton(
                                  child: Text(getTranslated(context, 'WITHD_WAL_TRANS_LBL')!,
                                      style: Theme.of(context)
                                          .textTheme
                                          .subtitle1!
                                          .copyWith(color: Theme.of(context).colorScheme.fontColor.withOpacity(0.5))),
                  ...
                  Padding(
                      ...
                      child: Container(
                        ...
                        child: TextFormField(
                          ...
                          validator: (value) => validateField(value!, getTranslated(context, 'FIELD_REQUIRED')),
                          enabled: true,
                      ...
                      child: TextFormField(
                        ...
                        decoration: InputDecoration(
                          border: OutlineInputBorder(
                            ...
                            borderSide: BorderSide(color: Theme.of(context).colorScheme.gray, width: 1),
                          ),
                          focusedBorder: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(17),
                            borderSide: BorderSide(color: Theme.of(context).colorScheme.gray, width: 1),
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          CupertinoButton(
                              ...
                              child: Container(
                                  padding: EdgeInsetsDirectional.only(top: 10, bottom: 10, start: 25, end: 25),
                                  //width: double.maxFinite,
                                  height: 40,
                                  alignment: FractionalOffset.center,
                                  decoration: BoxDecoration(
  ...
  Future<void> sendWithdrawRequest() async {
    ...
    if (_isNetworkAvail) {
      try {
        var parameter = {USER_ID: CUR_MERCHANTID, AMOUNT: amtC1!.text.toString(), PAYMENT_ADD: bankDetailC!.text.toString()};
        ...
        if (!error) {
          ...
          userProvider.setBalance(double.parse(getdata["data"]).toStringAsFixed(0));
        }
        if (mounted)
          setState(() {
            _isProgress = false;
    ...
    if (_isNetworkAvail) {
      ...
      try {
        ...
        apiBaseHelper.postAPICall(addTransactionApi, parameter).then((getdata) {
          ...
          if (!error) {
            // CUR_BALANCE = double.parse(getdata["new_balance"]).toStringAsFixed(0);
            UserProvider userProvider = Provider.of<UserProvider>(context, listen: false);
            userProvider.setBalance(double.parse(getdata["new_balance"]).toStringAsFixed(0).toString());
            isWithdraw = false;
            _isLoading = true;
            amtC!.clear();
            offset = 0;
              ...
              onPressed: () {
                ...
                if (form.validate() && amtC!.text != '0') {
                  ...
                  if (payMethod == null) {
                    ...
                    if (payMethod!.trim() == getTranslated(context, 'STRIPE_LBL')!.trim()) {
                      stripePayment(int.parse(amtC!.text));
                    } else if (payMethod!.trim() == getTranslated(context, 'RAZORPAY_LBL')!.trim()) {
                      razorpayPayment(double.parse(amtC!.text));
                    } else if (payMethod!.trim() == getTranslated(context, 'PAYSTACK_LBL')!.trim()) {
  ...
  Future<void> paypalPayment(String amt) async {
    ...
    try {
      var parameter = {USER_ID: CUR_MERCHANTID, ORDER_ID: orderId, AMOUNT: amt};
  ...
  void paytmPayment(double price) async {
    ...
    var parameter = {AMOUNT: price.toString(), USER_ID: CUR_MERCHANTID, ORDER_ID: orderId};
  ...
  paystackPayment(BuildContext context, int price) async {
    ...
    String? merchant_email = context.read<UserProvider>().curMerchantEmail;
  ...
  razorpayPayment(double price) async {
    ...
    String? contact = settingsProvider.merchant_email;
    String? merchant_email = settingsProvider.merchant_email;

    double amt = price * 100;

    if (contact != '' && merchant_email != '') {
      if (mounted) {
        setState(() {
          _isProgress = true;
        });
      ...
      var options = {
        ...
        'prefill': {CONTACT: contact, EMAIL: merchant_email},
      };
      ...
      child: InkWell(
          ...
          child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
                ...
                const Divider(),
                Text("${getTranslated(context, 'ID_LBL')!} : ${withdTranList[index].id!}"),
        ...
        apiBaseHelper.postAPICall(getWithdrawReqApi, parameter).then((getdata) {
          ...
          if (!error) {
            ...
            if ((offset1) < total1) {
              ...
              tempList1 = (data as List).map((data) => TransactionModel.fromWithdrawJson(data)).toList();
  ...
  Future<void> getTransaction() async {
    ...
    if (_isNetworkAvail) {
      try {
        var parameter = {LIMIT: perPage.toString(), OFFSET: offset.toString(), USER_ID: CUR_MERCHANTID, TRANS_TYPE: WALLET};
        apiBaseHelper.postAPICall(getWalTranApi, parameter).then((getdata) {
          ...
          if (!error) {
            ...
            Provider.of<UserProvider>(context, listen: false).setBalance(getdata["balance"]);
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Consumer<UserProvider>(builder: (context, userProvider, _) {
                        return Text(getPriceFormat(context, double.parse(userProvider.curBalance))!,
                            style: Theme.of(context)
                                .textTheme
                                .headline6!
                                .copyWith(color: Theme.of(context).colorScheme.fontColor, fontWeight: FontWeight.bold));
                ...
                : withdTranList.isEmpty
                    ...
                    : Column(
                        ...
                        children: [
                          ...
                          ListView.builder(
                            shrinkWrap: true,
                            itemCount: (offset1 < total1) ? withdTranList.length + 1 : withdTranList.length,
                            physics: const NeverScrollableScrollPhysics(),
                            itemBuilder: (context, index) {
                              return (index == withdTranList.length && isLoadingmore)
                                  ? const Center(
                                      child: CircularProgressIndicator(
                                      color: colors.primary,
                                    ))
