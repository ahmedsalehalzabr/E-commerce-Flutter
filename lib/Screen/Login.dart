...
class _LoginPageState extends State<Login> with TickerProviderStateMixin {
  ...
  bool visible = false;
  String? merchantUser_password,
      merchantUser_phone1,
      merchantUser_name,
      merchant_email,
      merchant_id,
      merchantUser_id,
      merchantUser_isAdmin,
      city_id,
      region_id,
      merchantUser_pincode,
      merchantUser_location,
      latitude,
      longitude,
      accessToken,
      merchantUser_image;
  ...
  Future<void> getLoginUser() async {
    var parameter = {MERCHANTUSER_PHONE1: merchantUser_phone1, MERCHANTUSER_PASSWORD: merchantUser_password};
    log('getUserLogin parameter=$parameter');
    apiBaseHelper.postNumoAPICall(postNumoMerchantLoginApi, parameter).then((getdata) async {
      log("Login getLoginUser getdata****$getdata");
      ...
      var i = getdata;
      log('Login getLoginUser getdata=$getdata');
      merchant_id = i[MERCHANT_ID].toString();
      merchantUser_id = i[MERCHANTUSER_ID].toString();
      merchantUser_name = i[MERCHANTUSER_NAME];
      merchant_email = i[MERCHANT_EMAIL] ?? '';
      merchantUser_phone1 = i[MERCHANTUSER_PHONE1].toString();
      city_id = i[CITY_ID].toString();
      region_id = i[REGION_ID].toString();
      merchantUser_location = i[MERCHANTUSER_LOCATION];
      merchantUser_pincode = i[MERCHANTUSER_PINCODE];
      latitude = i[LATITUDE];
      longitude = i[LONGITUDE];
      merchantUser_image = i[MERCHANTUSER_IMAGE];
      accessToken = i[ACCESS_TOKEN];

      CUR_MERCHANTID = merchant_id;
      CUR_MERCHANTUSERID = merchantUser_id;

      log('login data retrived');
      ...
      log('login data userProvider merchantUser_phone1=$merchantUser_phone1');
      ...
      settingProvider.saveUserDetail(merchant_id!, merchantUser_id!, merchantUser_name, merchant_email, merchantUser_phone1!, city_id, region_id,
          merchantUser_location, merchantUser_pincode, latitude, longitude, merchantUser_image, accessToken, context);
      log('login data settingProvider');
      ...
      log('error on getLoginUser =$error');
  ...
  _setFav(String pid) async {
    ...
    if (_isNetworkAvail) {
      try {
        var parameter = {MERCHANT_ID: CUR_MERCHANTID, PRODUCT_ID: pid};
  ...
  saveForLater(String vid, String qty) async {
    ...
    if (_isNetworkAvail) {
      try {
        var parameter = {PRODATTVALUE_ID: vid, USER_ID: CUR_MERCHANTID, QTY: qty, SAVE_LATER: "1"};
        apiBaseHelper.postAPICall(manageCartApi, parameter).then((getdata) {
          bool error = getdata["error"];
          String? msg = getdata["message"];
          if (!error) {
    ...
    return Padding(
        ...
        child: Align(
          ...
          child: Text(
            ...
            style: Theme.of(context).textTheme.subtitle1!.copyWith(color: Theme.of(context).colorScheme.fontColor, fontWeight: FontWeight.bold),
          ),
        ));
  }
          ...
          children: <Widget>[
            InkWell(
              onTap: () {
                ...
                settingsProvider.setPrefrence(MERCHANT_ID, merchant_id!);
                settingsProvider.setPrefrence(MERCHANTUSER_PHONE1, merchantUser_phone1!);
                settingsProvider.setPrefrence(MERCHANTUSER_ISADMIN, merchantUser_isAdmin!);
              ...
              child: Text(getTranslated(context, 'FORGOT_PASSWORD_LBL')!,
                  style:
                      Theme.of(context).textTheme.subtitle2!.copyWith(color: Theme.of(context).colorScheme.fontColor, fontWeight: FontWeight.normal)),
            ),
          ],
        ));
  }
  ...
  getSignUpText() {
    return Padding(
      padding: const EdgeInsetsDirectional.only(bottom: 20.0, start: 25.0, end: 25.0, top: 10.0),
      child: Row(
