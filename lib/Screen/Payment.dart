...
class StatePayment extends State<Payment> with TickerProviderStateMixin {
  ...
  late bool cod, paypal, razorpay, paumoney, paystack, flutterwave, stripe, paytm = true, gpay = false, bankTransfer = true;
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () {
      paymentMethodList = [
        Platform.isIOS ? getTranslated(context, 'APPLEPAY') : getTranslated(context, 'GPAY'),
                          ...
                          child: Column(
                            ...
                            children: [
                              Consumer<UserProvider>(builder: (context, userProvider, _) {
                                return Card(
                                  elevation: 0,
                                  child: userProvider.curBalance != "0" && userProvider.curBalance.isNotEmpty && userProvider.curBalance != ""
                                      ? Padding(
                                          ...
                                          child: CheckboxListTile(
                                            ...
                                            onChanged: (bool? value) {
                                              if (mounted) {
                                                setState(() {
                                                  isUseWallet = value;
                                                  if (value!) {
                                                    if (totalPrice <= double.parse(userProvider.curBalance)) {
                                                      ...
                                                      remWalBal = 0;
                                                      usedBal = double.parse(userProvider.curBalance);
                                                      isPayLayShow = true;
                                                    }
                                            ...
                                            subtitle: Padding(
                                              ...
                                              child: Text(
                                                ...
                                                style: TextStyle(fontSize: 15, color: Theme.of(context).colorScheme.black),
                                              ),
                                            ),
                                          ),
                                        )
  ...
  dateCell(int index) {
    ...
    return InkWell(
      child: Container(
        decoration: BoxDecoration(borderRadius: BorderRadius.circular(10), color: selectedDate == index ? colors.primary : null),
        ...
        child: Column(
          ...
          children: [
            Text(
              ...
              style: TextStyle(color: selectedDate == index ? Theme.of(context).colorScheme.white : Theme.of(context).colorScheme.lightBlack2),
            ),
            Padding(
              ...
              child: Text(
                ...
                style: TextStyle(
                    ...
                    color: selectedDate == index ? Theme.of(context).colorScheme.white : Theme.of(context).colorScheme.lightBlack2),
              ),
            ),
            Text(
        ...
        if (date == tdDate) {
          if (timeSlotList.isNotEmpty) {
            for (int i = 0; i < timeSlotList.length; i++) {
              ...
              DateTime last =
                  DateTime(cur.year, cur.month, cur.day, int.parse(time.split(':')[0]), int.parse(time.split(':')[1]), int.parse(time.split(':')[2]));

              if (cur.isBefore(last)) {
                timeModel.add(RadioModel(isSelected: i == selectedTime ? true : false, name: timeSlotList[i].name, img: ''));
              }
            }
          }
        } else {
  ...
  Future<void> _getdateTime() async {
    ...
    if (_isNetworkAvail) {
      timeSlotList.clear();
      try {
        var parameter = {TYPE: PAYMENT_METHOD, USER_ID: CUR_MERCHANTID};
                ...
                if (selectedDate != null) {
                  ...
                  if (date == tdDate) {
                    ...
                    DateTime last = DateTime(
                        cur.year, cur.month, cur.day, int.parse(time.split(':')[0]), int.parse(time.split(':')[1]), int.parse(time.split(':')[2]));
